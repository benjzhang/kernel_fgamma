...
def preprocess_input(x, vae=False):
    x = x.astype('float32')
    if np.max(x)>1:
        x = x/255
    if np.ndim(x) >2 and vae == False:
...
# ------------------------------------------------------------------
def train_autoencoder_image_classes(name, epochs=50, encoding_dim = 256, save_path='saved_model/deep'):
    if 'MNIST' in name:
    ...
    image_dim = np.shape(x_train)[1]
    num_classes = np.shape(y_train)[1]
    
    X_train = np.concatenate((x_train, y_train), axis=1)
    X_test = np.concatenate((x_test, y_test), axis=1)
    
    encoder_input = keras.Input(shape=(image_dim+num_classes,))
    ...
    if "swallow" in save_path:
        ...
        decoded = layers.Dense(image_dim+num_classes, activation='sigmoid')(encoded)
    elif 'CIFAR10' in name:
        encoded = layers.Dense(1024, activation='relu')(encoder_input)
...
def train_autoencoder_submnfld(name, epochs=50, encoding_dim = 3, save_path='saved_model/deep'):
    if name == 'Mixture_of_student_t_submnfld':
        N_dim=12
        N_samples = 10000
        try:
    ...
    elif name == 'Mixture_of_gaussians_submnfld_ae':
        N_dim=12
        N_samples = 10000
        try:
...
def train_autoencoder_image(name, epochs=50, encoding_dim = 256, save_path='saved_model/deep'):
    if 'MNIST' in name:
        import keras.datasets.mnist as ds
        data_shape = (28,28,1)
    elif 'CIFAR10' in name:
...
if __name__ == "__main__":
    ...
    if 'MNIST' in name:
        # name = "MNIST_deep"
        train_autoencoder_image(name = name, epochs=100, encoding_dim = encoding_dim, save_path='saved_model/'+name+"_"+str(encoding_dim))
    elif 'CIFAR10' in name:
        train_autoencoder_image(name = name, epochs=100, encoding_dim = encoding_dim, save_path='saved_model/'+name+"_"+str(encoding_dim))
    elif 'submnfld' in name:
        train_autoencoder_submnfld(name = "Mixture_of_gaussians_submnfld_ae", epochs=100, encoding_dim = encoding_dim, save_path='saved_model/'+name+"_"+str(encoding_dim))
