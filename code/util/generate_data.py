...
def generate_invariant_lorenz63(
    ...
    try:
        from util.dnmc_proj.lorenz63_gendata import return_data
    except:
        from dnmc_proj.lorenz63_gendata import return_data
...
def generate_data(p):
    ...
    if p.dataset == 'Learning_gaussian':
        ...
        Y_ = generate_gaussian(size=(p.N_samples_P, p.N_dim), m=4.0, std=1.0, random_seed=p.random_seed+100) # initial
        
    elif p.dataset == 'Mixture_of_gaussians2':
        #from util.generate_data import generate_gaussian, generate_four_gaussians
        p.expname = p.expname+'_%.2f' % p.sigma_Q
        X_ = generate_four_gaussians(size=(p.N_samples_Q, p.N_dim), dist=8.0, std=p.sigma_Q, random_seed=p.random_seed) # target
        Y_ = 8.0*(2*np.random.uniform(size=(p.N_samples_P, p.N_dim)) - 1)
        #Y_ = generate_gaussian(size=(p.N_samples_P, p.N_dim), m=4.0, std=1.0, random_seed=p.random_seed+100) # initial
        
    elif p.dataset == 'Mixture_of_gaussians3':
        #from util.generate_data import generate_two_gaussians
        p.expname = p.expname+'_%.2f' % p.sigma_Q
        X_ = generate_two_gaussians(size=(p.N_samples_Q, p.N_dim), mus=[[-2,0],[2,0]], std=p.sigma_Q, p = [0.5, 0.5], random_seed=p.random_seed) # target
        Y_ = generate_two_gaussians(size=(p.N_samples_P, p.N_dim), mus=[[-2,2],[2,-2]],  std=p.sigma_Q,  p = [0.05, 0.95], random_seed=p.random_seed) # initial
        
    elif p.dataset == 'Mixture_of_gaussians4':
        #from util.generate_data import generate_two_gaussians
        p.expname = p.expname+'_%.2f' % p.sigma_Q
        X_ = generate_two_gaussians(size=(p.N_samples_Q, p.N_dim), mus=[[-2,0],[2,0]], std=p.sigma_Q, p = [0.5, 0.5], random_seed=p.random_seed) # target
        Y_ = generate_two_gaussians(size=(p.N_samples_P, p.N_dim), mus=[[-2,0],[2,0]],  std=p.sigma_Q,  p = [0.05, 0.95], random_seed=p.random_seed) # initial
        ...
        Y_ = 10.0*(2*np.random.uniform(size=(p.N_samples_P, p.N_dim)) - 1)
        #Y_ = generate_gaussian(size=(p.N_samples_P, p.N_dim), m=10.0, std=1.0, random_seed=p.random_seed+100) # initial
        ...
        if 'switch' in p.dataset: # change one label to another
            ...
            if p.N_conditions > 1:
                p.expname = p.expname+'_cond'
            else:
                p.expname = p.expname+'_uncond'
            X_, X_label = import_mnist(N=p.N_samples_Q, label=p.label, normalized=True, random_seed=p.random_seed) # target
            if p.N_samples_P == p.N_samples_Q:
                Y_label = X_label
            else:
                prop = [np.sum(X_label[:, j], keepdims=False)/p.N_samples_Q for j in range(np.shape(X_label)[1])]
                data = np.random.choice(np.shape(X_label)[1], p.N_samples_P, p = prop)
                Y_label = generate_one_hot_encoding(p.N_samples_P, np.shape(X_label)[1], p.random_seed, data)
            p.Y_label = Y_label
            p.X_label = X_label
        ...
        if 'switch' in p.dataset: # change one label to another 
            ...
            if p.N_project_dim == None:
                Y_ = generate_logistic(size=tuple([p.N_samples_P]+N_dim), loc=0.0, scale=1.0, random_seed=p.random_seed+100) # initial
            else:
                Y_ = projected_logistic(size=tuple([p.N_samples_P]+N_dim), loc=0.0, scale=1.0, random_seed=p.random_seed+100, proj_dim=p.N_project_dim) # initial
    ...
    elif p.dataset == 'Lorenz63':
        p.N_dim = 3
        p.expname = p.expname+'_%.1f-%.1f_%.1f' % (p.y0[0], p.y0[1], p.y0[2])
        
        X_ = generate_invariant_lorenz63(size=(p.N_samples_Q, p.N_dim), y0=p.y0, random_seed=p.random_seed) # target
        Y_ = generate_gaussian(size=(p.N_samples_P, p.N_dim), m=0.0, std=0.01, random_seed=p.random_seed+100) # initial
        
  
    
    if p.mb_size_P > p.N_samples_P:
        p.mb_size_P = p.N_samples_P
    if p.mb_size_Q > p.N_samples_Q:
        p.mb_size_Q = p.N_samples_Q
